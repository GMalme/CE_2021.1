...
A seguir foi obtido um gráfico de 3 campos \ref{fig:enzodevs2000:BDP:3plotRAK}, no qual, pode-se visualizar à esquerda as maiores citações, no meio os autores que mais contribuíram à direita as palavras-chaves mais utilizadas no registro.
...
Também foi registrado um gráfico de 3 campos , no qual, à esquerda mantêm-se as citações e no meio os autores, já, á direita coloca-se os países dos autores. Dessa forma, chega-se ao seguinte gráfico \ref{fig:enzodevs2000:BDP:3plotRAC}:

\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{experiments/enzodevs2000/AnaliseBibliometrica/BigDataInPolicy/Graficos/references_author_countries.png}
    \caption{Gráfico de 3 campos envolvendo os parâmetro Citações, Autores, Países do \dataset\ BDP@enzodevs2000}
    \label{fig:enzodevs2000:BDP:3plotRAC}
\end{figure}

\subsection{Interpretações dos gráficos de 3 campos}

No primeiro gráfico \ref{fig:enzodevs2000:BDP:3plotRAK} verifica-se que a palavra-chave mais citada é \textbf{big data}, justamente a principal desta pesquisa. Entretanto, verifica-se, ainda, palavras como: \textit{privacy}, \textit{security} e \textit{social media}.

Tal associação pode indicar o uso de big data de forma que atinja esses aspectos. Como já foi mencionado na seção \ref{enzodevs2000:interpret_cresc}
